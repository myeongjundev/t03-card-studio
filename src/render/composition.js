 ...
 * 미니홈피는 사진첩 칸 안에, 필름은 필름 프레임 안에, 숏폼은 화면 전체에
 * 이미지가 들어간다.
 *
...
const IMAGE_BOX_BY_ERA = {
  ...
  // 필름: 좌우 스프로킷 홀을 피해 가운데 프레임 안에 넣는다.
  2012: { x: 0.14, y: 0.22, width: 0.72, height: 0.42 },
};
...
export function getComposition(state, width, height) {
  ...
  if (!ratios) return { key, era, imageBox: full(width, height) };
