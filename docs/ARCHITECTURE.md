...
### 스타일 프리셋 (`src/state/presets.js`)

슬라이더를 하나하나 맞춰 예쁘게 뽑는 사람은 많지 않다.
`예능 자막 / 감성 / Y2K / 미니멀` 을 한 번 눌러 적용한다.

두 가지를 지켰다.

- **문구와 이미지는 건드리지 않는다.** 눌렀더니 쓰던 문구가 사라지면 도구를
  다시 쓰지 않는다. 화면 비율도 바꾸지 않는다 — 캔버스 모양까지 변하면
  무엇이 바뀐 것인지 알기 어렵다.
- **템플릿 CRUD 를 대신하지 않는다.** 프리셋은 편집의 시작점일 뿐이고,
...
### 세 비율 한 번에 저장 (`io/exportAllRatios.js`)

카드 하나로 인스타 피드(1:1)·스토리(9:16)·릴스 표지(4:5)를 동시에 준비하는
경우가 흔하다. 매번 비율을 바꾸고 다운로드하기를 세 번 반복하는 대신
한 번에 세 파일을 받는다.

지금 화면에 보이는 비율이 아닌 나머지 두 비율은 미리보기 캔버스가 없으므로
`document.createElement('canvas')` 로 임시 캔버스를 만들어 그린다. 이때
두 가지를 미리보기와 반드시 맞춰야 한다.

- **같은 `renderCard` 를 쓴다.** 비율별로 렌더링 코드를 따로 두면 반드시
  갈라진다. 카드 2 의 핵심 불변식과 같은 이유다.
- **컨텍스트 설정도 같게 만든다.** 미리보기 캔버스는 가독성 검사 때문에
  `willReadFrequently: true` 로 만들어져 CPU 래스터화를 쓴다. 이 값이 다르면
  브라우저가 GPU 로 그려 글자 가장자리가 미세하게 달라진다 — 실제로 겪었던
  문제라 (`docs/TEST-EDGE-CASES.md` 4장) 처음부터 맞춰 두었다.

임시 캔버스는 DOM 에 붙이지 않는다. `toBlob` 은 DOM에 없는 캔버스에서도
동작하므로 붙일 필요가 없고, 붙이지 않으면 화면에 있는 캔버스 개수가 항상
미리보기 하나로 유지된다.

세 번의 다운로드를 곧바로 이어서 트리거하면 일부 브라우저가 반복되는
다운로드를 막는다. 파일 사이에 350ms 간격을 둔다.

검증: 1:1 로 저장된 파일과 미리보기 캔버스를 서브픽셀까지 대조해 차이 0 을
확인했다. 배치 도중 화면에 표시된 비율이 흔들리지 않는 것과, 임시 캔버스가
DOM 에 남지 않는 것도 확인했다.
