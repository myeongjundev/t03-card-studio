...
모습은 오랫동안 주로 **배경색**을 바꿨다. 그런데 사진을 올리면 배경색이
보이지 않는다. 사진 위에 문구를 얹는 것이 이 도구의 기본 사용법이라,
정작 그때 모습을 골라도 바뀌는 것이 거의 없었다. 픽셀로 재 보니 세 모습의
차이가 **2~9%** 였다 — 사실상 같은 카드다.

그래서 사진 위에도 남는 층을 모습마다 다르게 깐다(`drawPersonaEmphasis`).

| 모습 | 문구를 받치는 방식 |
|---|---|
| 기본 | 아무것도 깔지 않는다. 담백하게 두는 것이 그 모습이다 |
| 소셜 | 아래를 크게 눌러 주는 그라데이션. 피드에서 눈에 걸리도록 |
| 친한 친구 | 문구만 감싸는 둥근 쪽지. 화면 전체가 아니라 글자에만 붙는다 |

2004 는 예외로 두고 받침을 깔지 않는다. 밝은 싸이월드 페이지 위에 어두운
받침을 얹으면 어두운 다이어리 글이 묻힌다(실제로 대비가 1.5:1 까지 떨어졌다).
이 시대는 **페이지 스킨**이 대신 모습을 말한다 — 페이지 색, 제목, 강조색,
기분 단어, 그리고 `비공개` / `일촌평 12` 배지. 싸이월드에는 실제로 스킨이
있었으니 억지스럽지도 않다.

시대 장식과 겹치지 않는다. **시대는 "언제", 모습은 "누구에게"** 를 맡는다.
그래서 조합이 늘어나도 서로를 덮어쓰지 않는다.

지금은 사진을 올린 상태에서 47~58%(소셜 대비), 6~13%(친한 친구 대비)가
달라진다. e2e 에서 세 쌍 모두 3% 를 넘는지 확인한다.
