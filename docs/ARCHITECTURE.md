...
| 만드는 것 | 방법 |
|---|---|
| 화소가 굵게 뭉침 | 180px 버퍼로 줄였다가 `imageSmoothingEnabled = false` 로 되돌린다 |
| 색 띠 | 채널당 10단계로 양자화. 하늘·살색에 띠가 진다 |
| 저조도 노이즈 | 어두운 픽셀일수록 크게. 밝은 곳은 깨끗하다 |
| 초록 기울기 | 화이트밸런스가 늘 초록으로 기울어 있었다 |

**무거운 픽셀 연산은 작게 줄인 버퍼에서만 한다.** 1080×1080 원본에 직접
하면 100만 번이 넘어 편집할 때마다 버벅이지만, 180px 버퍼는 2만 번이
안 된다. 어차피 결과가 저화질이라 원본 해상도로 계산할 이유도 없다.

보간(`imageSmoothingEnabled`)을 반드시 꺼야 한다. 켜 두면 화소가 부드럽게
뭉개져서 "저화질" 이 아니라 그냥 "흐린 사진" 이 된다.

노이즈 세기는 두 번 조정했다. 처음 값(진폭 92, 6단계)에서는 피사체가
사라지고 모래폭풍만 남았다. 피처폰 사진도 무엇을 찍었는지는 알아볼 수
있어야 한다. 지금은 진폭 26, 10단계다.
